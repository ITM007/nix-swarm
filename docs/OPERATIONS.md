...
The deploy helper validates machine modules before remote mutation, syncs the managed source tree over SSH, and runs `nixos-rebuild switch` on each target host.
...
| Multiple replicas on one node | replicas exceed eligible live nodes |
