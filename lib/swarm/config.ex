defmodule Swarm.Config do
  ...
  @default_runtime %{
    ...
    executor: %{adapter: :fake, root: Path.join(System.tmp_dir!(), "swarm")},
    generation: "dev"
  }

  def current do
    raw =
      Application.get_env(:swarm, :cluster_config) ||
        load_from_path(
          Application.get_env(:swarm, :config_path) || System.get_env("SWARM_CONFIG_PATH")
        ) ||
        %{}
  ...
  def load_from_path(path) do
    case :file.consult(String.to_charlist(path)) do
      ...
      {:error, reason} ->
        raise "failed to read swarm config at #{path}: #{inspect(reason)}"
    end
  end
