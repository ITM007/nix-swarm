defmodule Swarm.CLI do
  ...
  def run(argv, tui_runner \\ &Swarm.TUI.run/1) do
    ...
    error in [Swarm.Remote.Error, ArgumentError, RuntimeError] ->
    ...
    if Keyword.has_key?(opts, :cookie) do
      IO.puts(
        ...
        "warning: passing --cookie on the command line exposes the cookie via `ps`/process listings; " <>
          "prefer --cookie-file, SWARM_COOKIE_FILE, or SWARM_COOKIE environment variables"
      )
    end
  end
  ...
  defp legacy_command_error(args) do
    ...
    Swarm is TUI-first in v0.1.0 alpha. Launch the console instead:
      swarm --target NODE
  ...
  defp print_help do
    ...
    Launch the operator TUI:
      swarm --target NODE
      swarm --target NODE --source /path/to/checkout

    Required:
      --target NODE              remote Swarm node to connect to
    ...
    TUI options:
      ...
      --source PATH              local Swarm checkout used for file editing/apply/update
    ...
    Notes:
      ...
      - Set SWARM_COOKIE_FILE once in your shell to keep launches short.
    """)
  end
end
