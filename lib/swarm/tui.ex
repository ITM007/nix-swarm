defmodule Swarm.TUI do
  ...
  alias Swarm.{Ascii, ClusterLogs, ConfigFiles, Deploy, Remote, Update}
  ...
  defp launch_service_action(state, action, service) do
    launch_job(state, {action, service}, fn ->
      ...
      results = Remote.rpc!(node, Swarm.API, service_action_api(action), [service])
  ...
  defp confirm_action_confirmation(%{action_confirmation: %{action: action, node: node}} = state) do
    launch_job(
      ...
      fn ->
        ...
        result = Remote.rpc!(target_node, Swarm.API, machine_action_api(action), [node])
      ...
      cond do
        ...
        true ->
          [
            ...
            Span.new(secondary_navigation_label(state), style: %Style{fg: :cyan})
          ]
      end
  ...
  defp secondary_navigation_label(%{active_view: :machines}),
    do:
      "arrows/hjkl move | mouse click/wheel | R reboot | Z shutdown | / search | t tail | ? help"

  defp secondary_navigation_label(%{active_view: :services}),
    do:
      "arrows/hjkl move | mouse click/wheel | b start | z stop | x restart | / search | t tail | ? help"

  defp secondary_navigation_label(%{active_view: :logs}),
    do: "arrows/hjkl move | mouse click/wheel | 1-4/f filter | / search | t tail | ? help"

  defp secondary_navigation_label(_state),
    do: "arrows/j/k move | mouse click/wheel | b start | z stop | x restart | ? help"
  ...
  defp overview_node_hostname(%{members: members}, node_name) when is_binary(node_name) do
    members
    |> Map.get(:deploy_hosts, %{})
    |> Enum.find_value(fn {configured_node, deploy_host} ->
      if Atom.to_string(configured_node) == node_name, do: deploy_host_hostname(deploy_host)
    end)
  end
