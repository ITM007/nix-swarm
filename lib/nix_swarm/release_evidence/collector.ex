defmodule NixSwarm.ReleaseEvidence.Collector do
  ...
  @default_gates [
    ...
    {:docker_hardened_matrix, ["docker", "compose", "--profile", "hardened", "ps"]},
    {:docker_reset, ["docker", "compose", "down", "-v", "--remove-orphans"]},
  ...
  def collect(opts \\ []) do
    ...
    docker_services =
      Keyword.get(opts, :docker_services, ["node-a", "node-b", "node-c", "operator"])

    started = elem(clock, 0).()

    results =
      Enum.map(gates, fn {name, command} ->
  ...
  defp collect_gate(:clean_checkout, command, _runner, clean?, clock, _required, _services) do
    result(
      :clean_checkout,
      command,
      if(clean?.(), do: :passed, else: :failed),
      if(clean?.(), do: "clean", else: "dirty_checkout"),
      nil,
      clock
    )
  end
  ...
  defp collect_gate(name, command, runner, _clean?, clock, require_docker, services) do
    ...
    try do
      case runner.(command) do
        {:unavailable, detail} ->
          result(name, command, :unavailable, detail, required, clock)
  ...
  defp runtime_result(name, output, 0, services)
       when name in [:docker_standard_matrix, :docker_hardened_matrix] do
  ...
  defp runtime_result(_name, output, status, _services),
    do: {:failed, "exit_status=#{status} #{String.trim(output)}"}
  ...
  defp result(name, command, status, detail, required, clock, exit_status \\ nil) do
    %{
      ...
      duration_ms: elapsed(clock, elem(clock, 0).()),
      detail: detail
    }
  end

  defp docker_gate?(name),
    do: name in [:docker_standard_matrix, :docker_hardened_matrix, :docker_reset]
