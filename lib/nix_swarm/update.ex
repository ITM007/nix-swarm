defmodule NixSwarm.Update do
  ...
  def run(opts, remote \\ nil, deploy_fun \\ &Deploy.run/1) when is_function(deploy_fun, 1) do
  ...
  def effective_deploy_opts(opts, cluster_state) do
    opts =
      opts
      |> normalize_opts()
      |> maybe_put_configurations(cluster_state)
  ...
  defp maybe_put_configurations(opts, %{overview: %{members: members}}) do
    case Map.get(members, :deploy_configurations, %{}) do
      configurations when map_size(configurations) > 0 ->
        Keyword.put_new(opts, :configurations, configurations)
  ...
  defp maybe_put_configurations(opts, _cluster_state), do: opts
