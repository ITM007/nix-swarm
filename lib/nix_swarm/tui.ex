defmodule NixSwarm.TUI do
  ...
  def run(opts) do
    Runtime.with_terminal_logging_suppressed(fn ->
  ...
  def with_terminal_logging_suppressed(fun) when is_function(fun, 0) do
    Runtime.with_terminal_logging_suppressed(fun)
  end

  @doc false
  def runtime_supported?(app_dir_fun \\ &Application.app_dir/2) do
    Runtime.supported?(app_dir_fun)
  end

  @doc false
  def runtime_support_error(app_dir_fun \\ &Application.app_dir/2) do
    Runtime.support_error(app_dir_fun)
  end

  defp ensure_runtime_supported!(app_dir_fun \\ &Application.app_dir/2) do
    unless Runtime.supported?(app_dir_fun) do
      raise RuntimeError, message: Runtime.support_error(app_dir_fun)
    end
  end
  ...
  def mount(opts) do
    ...
    state =
      State.new(
        Keyword.put(opts, :resume_state, resume_state),
