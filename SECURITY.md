...
Nix-Swarm is currently in the `0.3.x` alpha series. Security fixes are only guaranteed for the latest `v0.3.x` tag on `main`.

Older tags should be treated as unsupported once a newer `v0.3.x` release is published.
